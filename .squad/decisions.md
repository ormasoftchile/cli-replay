...
### Integration Analysis: cli-replay × gert

**Author:** Robert (Integration Architect)  
**Date:** 2026-04-03  
**Status:** Analysis / Recommendation

**Critical finding:** Every functional package lives under `internal/`. External Go modules **cannot import** any of these. Only `cmd` is importable (exports `Execute()` and `ValidationResult`).

**Integration patterns evaluated:**

1. **Pattern A: CLI Wrapper (Recommended for now)**
   - gert shells out to `cli-replay exec` with `--format json`
...
1. **Public `pkg/` API** — Promote scenario, runner, verify packages
...
Recommend **Pattern A (CLI wrapper)** for immediate integration. Roadmap contribution toward **Pattern B (library API)** for high-value upstream engagement. CLI interface is well-designed with structured JSON output, making it a solid integration surface. The critical gap is the `internal/` boundary — everything gert would want to import is locked behind it.
